...
for i, z0 in enumerate(z):
 for j, z1 in enumerate(z):
  if(i<j)and(abs(z0-z1)<r):
    edges.append([i,j])    
G=nx.Graph(edges)
#nx.draw(G)
...
for clique in cliques:
  ...
  print("d=", d)
  nullspace=[]
  ## option1: set threshold
  for di, d0 in enumerate(d):
    if(abs(d0)<thres):
      nullspace.append(p[:,di])
  # option2: get k_op lowest-eigenvalue operators 
  #k_op=1
  #arg=np.argsort(np.abs(d))
  #for i in range(k_op):
  #  di=arg[i]
  #  print("adding di=", di, "d=", d[di])
  #  nullspace.append(p[:,di])
